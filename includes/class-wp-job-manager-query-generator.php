...
class WP_Job_Manager_Query_Generator extends WP_Job_Manager_Listing_Query_Generator {
	/**
	 * Query arguments used to create the `WP_Query` object. Generated by the `WP_Resume_Manager_Query_Generator::get_query_args` method.
	 *
	 * @var array
	 */
	private $query_args;
	...
	protected function get_query_args() {
		...
		$args             = $this->get_args();
		$this->query_args = self::get_default_query_args();

		// Common arguments.
		$this->query_args['order']          = $this->parse_order();
		$this->query_args['orderby']        = $this->parse_orderby();
		$this->query_args['offset']         = $this->parse_offset();
		$this->query_args['posts_per_page'] = $this->parse_posts_per_page();
		$this->query_args['fields']         = $this->parse_fields();
		$this->query_args['no_found_rows']  = $this->parse_no_found_rows();

		// Job listing specific arguments.
		$this->query_args['s']           = $this->parse_keyword();
		$this->query_args['post_status'] = $this->parse_post_status();
		$this->query_args['meta_query']  = $this->generate_meta_query();
		$this->query_args['tax_query']   = $this->generate_tax_query();

		/** This filter is documented in wp-job-manager.php */
		$this->query_args['lang'] = apply_filters( 'wpjm_lang', null );
		 ...
		 */
		$this->query_args = apply_filters( 'job_manager_get_listings', $this->query_args, $args );
		...
		foreach ( $remove_empty as $query_key ) {
			if ( empty( $this->query_args[ $query_key ] ) ) {
				unset( $this->query_args[ $query_key ] );
			}
		}

		foreach ( $remove_null as $query_key ) {
			if ( null === $this->query_args[ $query_key ] ) {
				unset( $this->query_args[ $query_key ] );
			}
		}
