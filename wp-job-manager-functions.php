...
if ( ! function_exists( 'get_job_listings_keyword_search' ) ) :
	...
	function get_job_listings_keyword_search( $search, $wp_query ) {
		...
		if ( ! function_exists( 'job_manager_construct_secondary_conditions' ) && ! function_exists( 'job_manager_construct_post_conditions' ) ) {
			 ...
			 *
			 * @since $$next-version$$ Clauses now also match the entity-encoded form of the search term,
			 *                           so listings remain searchable whether `post_title`/`post_content`
			 *                           is stored raw or kses-encoded (e.g. `&` vs `&amp;`). The
			 *                           normalized needle is generated by WordPress's entity encoder,
			 *                           which also preserves already encoded entities. See #3030.
			 * @return array The SQL clauses.
			 */
			function job_manager_construct_post_conditions( $search_term, $is_excluding, $wildcard_search, $search_columns ) {
				...
				/*
				 * WordPress runs job titles and content through `wp_filter_kses` for users without
				 * the `unfiltered_html` capability, so a bare `&` is stored as `&amp;`. Build LIKE
				 * patterns for both the raw term and its entity-normalized form so listings are found
				 * regardless of how they were stored. `wp_kses_normalize_entities()` encodes bare
				 * ampersands without double-encoding existing entities. Post meta and taxonomy values
				 * are not kses-normalized on save, so this dual-form matching is intentionally limited
				 * to post columns here. See #3030.
				 */
				$needles    = [ $search_term ];
				...
				foreach ( $search_columns as $search_column ) {
					...
					// phpcs:disable WordPress.DB.PreparedSQL.InterpolatedNotPrepared, WordPress.DB.PreparedSQL.NotPrepared, WordPress.DB.PreparedSQLPlaceholders.UnfinishedPrepare -- Variables are safe or escaped.
				...
				if ( ! empty( $allow_query_attachment_by_filename ) ) {
					...
					foreach ( $like_patterns as $like ) {
						$attachment_conditions[] = "sq1.meta_value $like_op %s";
					}
					// phpcs:disable WordPress.DB.PreparedSQL.InterpolatedNotPrepared, WordPress.DB.PreparedSQL.NotPrepared, WordPress.DB.PreparedSQLPlaceholders.UnfinishedPrepare -- Variables are safe or escaped.
					$conditions[] = $wpdb->prepare( '( ' . implode( " $inner_op ", $attachment_conditions ) . ' )', $like_patterns );
