	...
	<fieldset class="fieldset-logged_in">
		...
		<div class="field account-sign-in">
			<?php
				...
				/**
				 * Filter the logout URL for job creation form.
				 *
				 * @since 1.15.0
				 *
...
<?php else :
	...
	<fieldset class="fieldset-login_required">
		...
		<div class="field account-sign-in">
			<?php 
			/**
			 * Filters the login URL for job creation form.
			 * 
			 * @since 1.15.0
			 *
			...
			<?php elseif ( $account_required ) : ?>
				...
				<?php 
				/**
				 * Filters the login required message on job creation page.
				 *
				 * @since 1.31.1
				 *
				 * @param string $message Login required message.
				 */
				echo wp_kses_post( apply_filters( 'submit_job_form_login_required_message',  __( 'You must sign in to create a new listing.', 'wp-job-manager' ) ) ); ?>
	...
	if ( ! empty( $registration_fields ) ) {
		foreach ( $registration_fields as $key => $field ) {
			...
			<fieldset class="fieldset-<?php echo esc_attr( $key ); ?>">
				<?php 
				/**
				 * Filters the required label for account field on the job creation form.
				 *
				 * @since 1.19.0
				 *
				 * @param string $label Required label for account field.
				 */
				 ?>
				<label
		...
		/**
		 * Runs after account sign in area on job creation form.
		 * 
		 * @since 1.27.0
		 */
